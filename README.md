...
- **Scoped sudo, nothing more.** The installer writes a `visudo`-validated sudoers file granting the service user passwordless sudo for a fixed set of **exactly-argument** commands — no wildcards, no globs, no directories. Six are always granted:
  ...
  | `systemctl poweroff` | |
  | `systemctl suspend` | |
  | `systemctl restart plugin_loader` | revives Decky Loader's panel, which dies whenever Steam's CEF restarts and never comes back on its own |
  | `systemctl restart --no-block couchside.service` | lets an app-triggered update reload the new agent. Adds **no** privilege: that unit already runs the service user's own code as the service user |
  | *(root-owned journal wrapper)* | see below |

  Three more depend on what the box has. If a supported display manager is detected, the installer also grants `systemctl restart <that manager>` (the fix-a-black-screen rescue action) and a `tee` of **one fixed file path**, `/etc/<manager>.conf.d/zzz-couchside-session.conf`, which is how "boots into Game Mode / Desktop" is stored. On SDDM boxes a second `tee`, of the legacy `zz-` path, is granted so an updating box can blank its own stale pre-2.9.64 file. **Both `tee` grants are root-owned file writes**, and each names one exact path in the rule — this is not a grant of `tee` in general, and it cannot write anywhere else. A box with no supported manager gets **none** of the three: the agent degrades closed and the button never renders.

  Note the journal grant carefully: it is for the **wrapper**, never for `journalctl` itself. The wrapper validates the unit and the line count and calls `journalctl` with a locked-down option set, because *any* wildcard rule on `journalctl` would also permit `--file` / `--directory` — turning "read the journal" into arbitrary file reads as root.
...
- **The privileged helper (agent 2.9.69+).** Newer installs also place a small root process, `couchside-helper.py`, socket-activated by systemd. It exists to shrink the sudoers surface above, not to widen it: the same privileged operations are reached through a **frozen verb table** of ten entries (`session.set-boot`, `session.clear-boot`, `dm.restart`, `power`, `unit.restart`, `logs.journal`, `update.flatpak`, `update.os`, — helper 1.1.0+ — `decky.loader`, which only starts the pinned Decky Loader unit described below and refuses without the opt-in marker, and — helper 1.2.0+ — `usb.wake-arm`, which writes `enabled`/`disabled` to a USB device's `power/wakeup` after validating the device id by membership in the kernel's own device listing, never interpolating it into the path), and an unknown verb runs nothing. It never sees a command, a path, or a shell string from a client — a verb selects a handler, and any argument is validated against a closed set before use.
